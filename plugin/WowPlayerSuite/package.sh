#!/usr/bin/env bash
#
# WowPlayerSuite 插件打包脚本
#
# 产出可直接安装/发布的 zip：解压后为顶层 WowPlayerSuite/ 目录，放进
# Interface/AddOns/ 即可使用。
#
# 用法:
#   ./package.sh              # 打包到 plugin/dist/WowPlayerSuite-<version>.zip
#   ./package.sh --check      # 只做校验，不打包
#   ./package.sh --output DIR # 指定输出目录
#
# 校验项:
#   1. WowPlayerSuite.toc 存在且含 ## Version / ## Interface
#   2. Constants.lua 的 C.VERSION 与 .toc 的 Version 一致
#   3. .toc 引用的每个文件都真实存在
#   4. WowPlayerSuite/ 下的 .lua 是否有未被 .toc 引用（仅警告）

set -euo pipefail

# --- 路径解析：脚本所在目录即插件目录 plugin/WowPlayerSuite -----------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ADDON_NAME="WowPlayerSuite"
...
CHECK_ONLY=0
# 统一插件产出目录：plugin/dist（WowPlayerSuite 的上一级）。
...
[[ -n "$TOC_VERSION" ]]   || fail "WowPlayerSuite.toc 缺少 ## Version:"
[[ -n "$TOC_INTERFACE" ]] || fail "WowPlayerSuite.toc 缺少 ## Interface:"
...
if [[ -f "$CONSTANTS_FILE" ]]; then
  ...
  if [[ -n "$CONST_VERSION" && "$CONST_VERSION" != "$TOC_VERSION" ]]; then
    fail "版本不一致: WowPlayerSuite.toc=$TOC_VERSION, Constants.lua=$CONST_VERSION"
  fi
...
# 在插件上级目录打包，使 zip 顶层为 WowPlayerSuite/；排除系统与开发垃圾文件。
(
  cd "$PARENT_DIR"
