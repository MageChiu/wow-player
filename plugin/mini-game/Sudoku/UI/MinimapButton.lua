...
local function showTooltip(btn)
  ...
  GameTooltip:AddLine("闲下来玩一局数独。", 0.8, 0.8, 0.8)
  if ns.Game and ns.Game:HasGame() and not ns.Game:IsWon() then
    local diff = Const.DIFFICULTY_BY_KEY[ns.Game:GetDifficulty()]
    GameTooltip:AddDoubleLine("进行中",
      (diff and diff.label or "?") .. "  剩余 " .. ns.Game:RemainingCount(),
      0.6, 1, 0.6, 1, 1, 1)
  end
  GameTooltip:AddLine(" ")
  GameTooltip:AddLine("左键: 打开数独", 0.4, 0.8, 1)
...
function MB:_ensureButton()
  ...
  local btn = CreateFrame("Button", "SudokuMinimapButton", Minimap)
  btn:SetSize(32, 32)
