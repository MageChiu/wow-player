#!/usr/bin/env bash
#
# QuickDisenchant 插件打包脚本
#
# 产出可直接安装/发布的 zip：解压后为顶层 QuickDisenchant/ 目录，放进
# Interface/AddOns/ 即可使用。
#
# 用法:
#   ./package.sh              # 打包到 plugin/dist/QuickDisenchant-<version>.zip
#   ./package.sh --check      # 只做校验，不打包
#   ./package.sh --output DIR # 指定输出目录
#
# 校验项:
#   1. QuickDisenchant.toc 存在且含 ## Version / ## Interface
#   2. Constants.lua 的 C.VERSION 与 .toc 的 Version 一致
#   3. .toc 引用的每个文件都真实存在
#   4. QuickDisenchant/ 下的 .lua 是否有未被 .toc 引用（仅警告）

set -euo pipefail

# --- 路径解析：脚本所在目录即 plugin/trade/quick-disenchant ------------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ADDON_NAME="QuickDisenchant"
ADDON_DIR="${SCRIPT_DIR}/${ADDON_NAME}"
TOC_FILE="${ADDON_DIR}/${ADDON_NAME}.toc"
CONSTANTS_FILE="${ADDON_DIR}/Constants.lua"

CHECK_ONLY=0
# 统一插件产出目录：plugin/dist（trade 的上一级的上一级）。
...
[[ -n "$TOC_VERSION" ]]   || fail "QuickDisenchant.toc 缺少 ## Version:"
[[ -n "$TOC_INTERFACE" ]] || fail "QuickDisenchant.toc 缺少 ## Interface:"
...
if [[ -f "$CONSTANTS_FILE" ]]; then
  ...
  if [[ -n "$CONST_VERSION" && "$CONST_VERSION" != "$TOC_VERSION" ]]; then
    fail "版本不一致: QuickDisenchant.toc=$TOC_VERSION, Constants.lua=$CONST_VERSION"
  fi
...
# 在本目录内打包，使 zip 顶层为 QuickDisenchant/；排除系统与开发垃圾文件。
(
  cd "$SCRIPT_DIR"
