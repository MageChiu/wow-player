...
# MyChat 插件打包脚本
#
# 产出可直接安装/发布的 zip：解压后为顶层 MyChat/ 目录，放进
# Interface/AddOns/ 即可使用。
#
# 用法:
#   ./package.sh              # 打包到 dist/MyChat-<version>.zip
#   ./package.sh --check      # 只做校验，不打包
#   ./package.sh --output DIR # 指定输出目录
#
# 校验项:
#   1. MyChat.toc 存在且含 ## Version / ## Interface
#   2. Constants.lua 的 C.VERSION 与 .toc 的 Version 一致
#   3. .toc 引用的每个文件都真实存在
#   4. MyChat/ 下的 .lua 是否有未被 .toc 引用（仅警告）

set -euo pipefail

# --- 路径解析：脚本所在目录即 plugin/chat -----------------------------------
...
CHECK_ONLY=0
# 统一插件产出目录：plugin/dist（chat 的上一级）。所有插件的 zip 汇总于此。
...
[[ -n "$TOC_VERSION" ]]   || fail "MyChat.toc 缺少 ## Version:"
[[ -n "$TOC_INTERFACE" ]] || fail "MyChat.toc 缺少 ## Interface:"
...
if [[ -f "$CONSTANTS_FILE" ]]; then
  ...
  if [[ -n "$CONST_VERSION" && "$CONST_VERSION" != "$TOC_VERSION" ]]; then
    fail "版本不一致: MyChat.toc=$TOC_VERSION, Constants.lua=$CONST_VERSION"
  fi
...
# 在 chat 目录内打包，使 zip 顶层为 MyChat/；排除系统与开发垃圾文件。
(
  cd "$SCRIPT_DIR"
